...
public class UNETChat : Chat
{
    ...
    {
        // Maybe Change
        if (Input.GetKeyDown(KeyCode.KeypadEnter) && gameObject.GetComponentInChildren<UnityEngine.UI.InputField>().text.Length > 0)
        {
            SendMessage(gameObject.GetComponentInChildren<UnityEngine.UI.InputField>());
        }
    }
	...
	private void ServerReceiveMessage(NetworkMessage message)
	{
        ...
        myMessage.value = GameObject.FindGameObjectWithTag("NetworkSpawning").GetComponent<PlayerConnectionObject>().PlayerName + ": " + message.ReadMessage<StringMessage>().value;
